...
int main() {
    ...
    if (fd < 0) {
        std::cerr << "prod: Error while opening a file due to error: " << strerror(errno) << std::endl;
        exit(errno);
    }
    ...
    if (result < 0) {
        std::cerr << "prod: Error while truncating a file due to error: " << strerror(errno) << std::endl;
        exit(errno);
    }
    ...
    if (addr == MAP_FAILED) {
        std::cerr << "prod: Error while mapping memory due to error: " << strerror(errno) << std::endl;
        exit(errno);
    }
    ...
    if (is_full_sem == SEM_FAILED) {
        std::cerr << "prod: Error while opening semaphore is_full due to error: " << strerror(errno) << std::endl;
        exit(errno);
    }
    ...
    if (is_empty_sem == SEM_FAILED) {
        std::cerr << "prod: Error while opening semaphore is_empty due to error: " << strerror(errno) << std::endl;
        exit(errno);
    }
    ...
    if (memory_sem == SEM_FAILED) {
        std::cerr << "prod: Error while opening semaphore for shared memory due to error: " << strerror(errno) << std::endl;
        exit(errno);
    }
    ...
    // the loop of printing characters in file
    while(true) {
        sem_wait(is_full_sem);
        sem_wait(memory_sem);
        
        std::size_t written = write(fd, buffer, 1);
            if (written < 1) {
                std::cerr << "prod: Error while writing due to error: " << strerror(errno) << std::endl;
                exit(errno);
            }
        ++(*offset);
